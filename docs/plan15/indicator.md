...
我们的 demo 和 API 太简单了，这个组件在 ant-design 里没有直接对应组件，请参考 Swap 这次的改造方式，把 Rue 的 Indicator 做成同样的增强风格：保持 Rue 自己的视觉风格，不照搬其他库的视觉，但 API 和能力可以增强，尽可能补齐核心能力；保留并融合现有 demo，不删除旧 demo，只做补强和重组；一次性写完；只修改 Indicator 相关文件，不要动其他组件。
...
1. 只改 Indicator 相关文件，不要顺手修改其他组件。
