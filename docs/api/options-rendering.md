...
- **详情**
  ...
  通过 `template` 选项提供的模板将在运行时即时编译。仅在使用包含模板编译器的 Rue 构建时支持。模板编译器**不包含**在名称中有 `runtime` 的 Rue 构建中，例如 `rue.runtime.esm-bundler.js`。有关不同构建的更多详细信息，请参阅 [dist 文件指南](https://github.com/rues/core/tree/main/packages/rue#which-dist-file-to-use)。
