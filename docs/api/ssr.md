...
- **示例**
  ...
  ```js
  import { createSSRApp } from 'rues'
  import { renderToString } from 'rues/server-renderer'
...
- **示例**
  ...
  :::tip 注意
  此方法在从 Node.js 环境解耦的 `rues/server-renderer` 的 ESM 构建中不受支持。请改用 [`pipeToNodeWritable`](#pipetonodewritable)。
  :::
...
- **示例**
  ...
  ```tsx
  import { useSSRContext } from 'rues'
