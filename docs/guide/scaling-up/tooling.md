...
$ cd my-rue-app
$ npm i @rue/runtime @rue/vite-plugin-rue
```

```sh [pnpm]
$ pnpm create vite@latest my-rue-app -- --template vanilla-ts
$ cd my-rue-app
$ pnpm add @rue/runtime @rue/vite-plugin-rue
```

```sh [yarn]
$ yarn create vite my-rue-app --template vanilla-ts
$ cd my-rue-app
$ yarn add @rue/runtime @rue/vite-plugin-rue
```

```sh [bun]
$ bun create vite my-rue-app --template vanilla-ts
$ cd my-rue-app
$ bun add @rue/runtime @rue/vite-plugin-rue
```
...
- Rue 的编译与指令由 `@rue/vite-plugin-rue` 负责集成与优化
...
- Rue 官方提供的 `@rue/vite-plugin-rue` 覆盖了常见用例，扩展场景可自行添加插件

## 底层包与生态

以下包在 Rue 项目中常用：

- `@rue/runtime`：Rue 运行时核心
- `@rue/shared`：通用工具与类型
- `@rue/vite-plugin-rue`：Rue 与 Vite 集成的官方插件
- `@rue/router`：路由能力
- `@rue/jsx-runtime`：JSX 相关运行时支持
- `@rue/runtime-vapor`：针对特定运行模式的优化实现
