...
在实际项目中，我们通常将 SFC 编译器与构建工具（如 [Vite](https://vitejs.dev/)）集成，Rue 通过 `@rue/vite-plugin-rue` 提供官方插件支持。查看 [SFC 工具链](/guide/scaling-up/tooling) 部分了解更多详情。
...
const filtered = computed(() =>
  props.items.filter(item => item.includes(search.value)),
)
</script>
...
```tsx
import { ref, computed, type FC } from 'rues'
