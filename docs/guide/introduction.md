...
```tsx
import { type FC, ref, reactive, computed } from 'rues'
...
```tsx
import { type FC, useState } from 'rues'
...
```tsx
import { type FC, ref, watch, watchEffect } from 'rues'
...
const Child: FC<{
  ...
  return (
    <div>
      <p>{props.message}</p>
      <button onClick={() => props.onNotify?.('来自子组件的消息')}>
        通知父组件
      </button>
    </div>
  )
