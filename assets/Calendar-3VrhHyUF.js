...
<\/script>`}),v,P)});let F=n(`section`);d(v,F),c(F,`my-12 space-y-6`);let I=n(`div`);d(F,I);let L=n(`h2`);d(I,L),c(L,`mb-2`),d(L,i(`API`));let R=n(`p`);d(I,R),c(R,`m-0 text-sm text-base-content/70`),d(R,i(`默认 Calendar 面板聚焦于 ant-design Calendar 的核心交互能力，同时保留 Rue 自己的视觉和旧导出方式。`));let z=o(`rue:component:anchor`);d(F,z),m(()=>{u(_(we,{rows:pe}),F,z)});let ee=n(`div`);d(F,ee);let B=n(`h2`);d(ee,B),c(B,`mb-2`),d(B,i(`附属导出`));let V=n(`p`);d(ee,V),c(V,`m-0 text-sm text-base-content/70`),d(V,i(`历史上的 Cally 与 Pikaday 接口没有删除，而是并列保留为复合导出，便于渐进迁移。`));let H=o(`rue:component:anchor`);return d(F,H),m(()=>{u(_(Te,{rows:me}),F,H)}),h})}),h,y),h})};export{Ue as default};