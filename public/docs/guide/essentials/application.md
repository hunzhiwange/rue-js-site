...
`.mount()` 方法应在所有应用配置和资源注册完成后调用。与资源注册方法不同，`.mount()` 返回 `void`，不返回应用实例或根组件实例。

同一个应用实例重复挂载到同一个容器时，Rue 会将后续调用视为幂等操作，不会重新渲染或重复触发生命周期。应用挂载后会独占该容器；如果要将应用迁移到其他容器，或让另一个应用使用当前容器，请先调用 `.unmount()`。重复调用 `.unmount()` 同样是安全的幂等操作。
