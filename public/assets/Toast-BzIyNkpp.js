...
<div className="relative min-h-80 overflow-hidden rounded-box border border-base-300">
  ...
  <Toast.Item
    ...
    closable
  />
</Toast>`});h(()=>u(e,c,I))});let L=g(`rue:component:anchor`);e(c,L),t(()=>{let e=m(D,{title:`Variants and actions`,summary:`soft、outline、solid 三种外观可以调整存在感，action 让单条提示具备 message 之上的轻量操作能力。`,tab:r.variants,preview:()=>b(`div`,{className:`grid gap-4`,children:re.map(e=>x(A,{minHeight:`11rem`,children:[b(`div`,{className:`absolute left-3 top-3 badge badge-ghost badge-sm`,children:e.label}),b(w,{className:`absolute`,placement:`bottom-start`,inset:{x:12,y:12},children:b(w.Item,{type:e.type,variant:e.variant,title:`${e.label} feedback`,description:`Toast.Item 可以直接承载业务动作。`,action:b(`button`,{type:`button`,className:`btn btn-xs btn-ghost`,children:`Undo`}),closable:!0})})]},e.label))}),code:`<Toast.Item
  type="warning"
  variant="outline"
  ...
  closable
/>`});h(()=>u(e,c,L))});let R=g(`rue:component:anchor`);e(c,R),t(()=>{let e=m(D,{title:`Controlled and auto close`,summary:`受控关闭适合和外部状态联动；自动关闭则提供了 message 常用的短时反馈体验，并支持 hover 暂停。`,tab:r.controlled,preview:()=>b(le,{}),code:ue});h(()=>u(e,c,R))});let z=g(`rue:component:anchor`);e(c,z),t(()=>{let e=m(D,{title:`Toast.useMessage`,summary:`参考常见 message hook 的形态：把 contextHolder 放进页面即可，真正的消息默认弹到全局页面层；如果你要留在当前 box，再显式传 getContainer={false}。`,tab:r.useMessage,preview:()=>x(A,{minHeight:`18rem`,children:[x(`div`,{className:`absolute inset-x-4 top-4 flex flex-wrap gap-2`,children:[b(`button`,{type:`button`,className:`btn btn-sm btn-primary`,onClick:()=>{o.open({type:`success`,content:`This is a prompt message for success, and it will disappear in 10 seconds`,duration:10})},children:`10 秒成功提示`}),b(`button`,{type:`button`,className:`btn btn-sm btn-outline`,onClick:()=>{o.loading({key:`publish`,content:`Publishing changes to preview...`})},children:`开始发布`}),b(`button`,{type:`button`,className:`btn btn-sm btn-outline`,onClick:()=>{o.open({key:`publish`,type:`success`,content:`Published to preview. Same key, same slot, new state.`,duration:2})},children:`更新同 key`}),b(`button`,{type:`button`,className:`btn btn-sm btn-ghost`,onClick:()=>{o.destroy()},children:`清空当前消息`})]}),_]}),code:`const [messageApi, contextHolder] = Toast.useMessage({
  placement: 'top-end',
  inset: { x: 16, y: 68 },
  gap: 12,
  maxCount: 3,
...
//   inset: { x: 12, y: 12 },
// })`});h(()=>u(e,c,z))});let B=g(`rue:component:anchor`);e(c,B),t(()=>{let e=m(D,{title:`Compound composition`,summary:`如果标题、说明和操作区需要更强定制，可以直接使用 compound 子组件自己拼装。`,tab:r.compound,preview:()=>b(A,{minHeight:`15rem`,children:b(w,{className:`absolute`,placement:`bottom-start`,inset:{x:16,y:16},children:x(w.Item,{variant:`outline`,className:`max-w-md`,children:[b(w.Icon,{className:`bg-secondary/12 text-secondary`,children:b(`span`,{className:`text-lg font-black`,children:`R`})}),x(w.Content,{children:[b(w.Title,{children:`Workspace synced`}),b(w.Description,{children:`Compound API 适合带结构化说明、额外按钮和自定义图标的业务提示。`}),x(`div`,{className:`mt-3 flex flex-wrap gap-2`,children:[b(`button`,{type:`button`,className:`btn btn-sm btn-primary btn-soft`,children:`Open changelog`}),b(`button`,{type:`button`,className:`btn btn-sm btn-ghost`,children:`Later`})]})]}),b(w.Action,{className:`ml-3 items-start self-start`,children:b(w.Close,{className:`text-base-content/50 hover:bg-base-200 hover:text-base-content`})})]})})}),code:`<Toast className="absolute" placement="bottom-start" inset={{ x: 16, y: 16 }}>
...
</Toast>`});h(()=>u(e,c,pe))});let me=g(`rue:component:anchor`);e(c,me),t(()=>{let e=m(D,{title:`Inset and layer control`,summary:`inset 负责把提示从边缘收进来，zIndex 用于压过页面中的局部浮层，适合嵌入 mock 页面或设计稿容器。`,tab:r.inset,preview:()=>x(A,{minHeight:`15rem`,children:[x(`div`,{className:`absolute inset-4 rounded-[1.25rem] border border-base-300 bg-base-100/80 p-4`,children:[b(`div`,{className:`h-10 rounded-box bg-base-200/80`}),x(`div`,{className:`mt-4 grid gap-3 md:grid-cols-2`,children:[b(`div`,{className:`h-20 rounded-box bg-base-200/70`}),b(`div`,{className:`h-20 rounded-box bg-base-200/70`})]})]}),b(`div`,{className:`absolute right-8 top-10 z-10 rounded-box border border-base-300 bg-base-100 px-4 py-3 text-xs shadow-sm`,children:`背景浮层`}),x(w,{className:`absolute`,placement:`top-end`,inset:{x:20,y:20},gap:10,zIndex:30,children:[b(`div`,{role:`alert`,className:`alert alert-success shadow-lg`,children:b(`span`,{children:`Layered above the card.`})}),b(`div`,{role:`alert`,className:`alert alert-info shadow-sm`,children:b(`span`,{children:`Inset keeps it off the edge.`})})]})]}),code:`<Toast
  className="absolute"
  placement="top-end"
  inset={{ x: 20, y: 20 }}
  gap={10}
...
</Toast>`});h(()=>u(e,c,me))});let he=g(`rue:component:anchor`);e(c,he),t(()=>{let e=m(D,{title:`Custom host element`,summary:`需要语义容器时，可以把根节点改成 section，并直接挂上 status / aria-live 之类的可访问性语义。`,tab:r.host,preview:()=>b(A,{children:x(w,{as:`section`,className:`absolute`,placement:`top`,inset:{x:16,y:16},role:`status`,"aria-live":`polite`,gap:10,children:[b(`div`,{role:`alert`,className:`alert alert-info shadow-sm`,children:b(`span`,{children:`Auto save completed.`})}),b(`div`,{role:`alert`,className:`alert alert-warning shadow-sm`,children:b(`span`,{children:`1 draft still requires review.`})})]})}),code:`<Toast
  as="section"
  className="absolute"
  placement="top"
  inset={{ x: 16, y: 16 }}
