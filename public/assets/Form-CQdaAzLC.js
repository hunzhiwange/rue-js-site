...
/>
`});T(()=>_(t,p,K))});let q=E(`rue:component:anchor`);r(p,q),i(()=>{let e=w(V,{title:`Form instance and reactive summary`,summary:`用 Form.useForm 和 Form.useWatch 把同一个表单实例共享给按钮和摘要面板。`,tab:t,preview:()=>P(Ee,{}),code:`const InstanceMethodsShowcase: FC = () => {
...
`});T(()=>_(e,p,q))});let J=E(`rue:component:anchor`);r(p,J),i(()=>{let e=w(V,{title:`Validation timing and dependencies`,summary:`把 validateTrigger、dependencies 和 warningOnly 放进一个更贴近注册/发布配置场景的表单里。`,tab:n,preview:()=>P(De,{}),code:`const [form] = Form.useForm()
...
/>
`});T(()=>_(e,p,J))});let ee=E(`rue:component:anchor`);r(p,ee),i(()=>{let e=w(V,{title:`Normalize and value mapping`,summary:`演示 getValueProps、normalize 和 messageVariables 在实际业务表单里的配合方式。`,tab:o,preview:()=>P(Oe,{}),code:`<Form.Item
  form={form}
  name="issueId"
  label="Issue 编号"
...
<Form.Item
  ...
  render={control => <Input {...control} />}
/>
`});T(()=>_(e,p,ee))});let te=E(`rue:component:anchor`);r(p,te),i(()=>{let e=w(V,{title:`Conditional fields with shouldUpdate`,summary:`用 shouldUpdate 按需挂载额外区域，而不是让每个字段都承担条件渲染职责。`,tab:s,preview:()=>P(ke,{}),code:`<Form.Item
...
`});T(()=>_(e,p,te))});let ne=E(`rue:component:anchor`);r(p,ne),i(()=>{let e=w(V,{title:`Composite controls with noStyle`,summary:`一个视觉行里组合多个字段时，用外层 Item 负责排版，内部 noStyle Item 只做字段绑定。`,tab:c,preview:()=>P(Ae,{}),code:`const [form] = Form.useForm()
...
`});T(()=>_(e,p,ne))});let re=E(`rue:component:anchor`);r(p,re),i(()=>{let e=w(V,{title:`Dynamic list`,summary:`用 Form.List 组织动态数组字段，并补上列表级规则、重排操作和 ErrorList。`,tab:m,preview:()=>P(Me,{}),code:`const DynamicListShowcase: FC = () => {
  const [form] = Form.useForm()
  const members =
...
`});T(()=>_(e,p,re))});let ie=E(`rue:component:anchor`);r(p,ie),i(()=>{let e=w(V,{title:`Long form scrolling`,summary:`在可滚动长表单里同时展示 scrollToField 和 scrollToFirstError 的定位行为。`,tab:h,preview:()=>P(je,{}),code:`const LongFormScrollShowcase: FC = () => {
  const [form] = Form.useForm()
  const activity = ref(
...
`});T(()=>_(e,p,ie))});let ae=E(`rue:component:anchor`);r(p,ae),i(()=>{let e=w(V,{title:`Advanced search`,summary:`把高级搜索表单整理成 Rue 实现：网格字段、独立按钮区、统一实例驱动。`,tab:b,preview:()=>P(Ne,{}),code:`const [form] = Form.useForm()
