...
/>`});m(()=>l(e,s,B))});let V=h(`rue:component:anchor`);e(s,V),t(()=>{let e=p(D,{title:`With steps and measure`,summary:`基础示例 保持，但把刻度线和标签收进 marks，减少样板代码。`,tab:v,preview:()=>b(M,{}),code:`<Range
  min={0}
  max={100}
  defaultValue={25}
  step={25}
...
/>`});m(()=>l(e,s,V))});let H=h(`rue:component:anchor`);e(s,H),t(()=>{let e=p(D,{title:`Range colors`,summary:`使用颜色矩阵，同时展示语义 API 与颜色类可以自然叠加。`,tab:x,preview:()=>b(N,{}),code:`const colors = ['neutral', 'primary', 'secondary', 'accent', 'success', 'warning', 'info', 'error'] as const
...
</div>`});m(()=>l(e,s,H))});let U=h(`rue:component:anchor`);e(s,U),t(()=>{let e=p(D,{title:`Sizes`,summary:`展示 xs 到 xl 的基础尺寸，同时补上 small / medium / large 三个别名，方便和其他组件对齐。`,tab:w,preview:()=>y(`div`,{className:`space-y-6`,children:[b(`div`,{className:`flex w-full max-w-sm flex-col gap-4`,children:T.map((e,t)=>b(S,{size:e,min:0,max:100,defaultValue:30+t*10},e))}),b(`div`,{className:`grid gap-3 rounded-box border border-dashed border-base-300 bg-base-100 p-4 text-sm sm:grid-cols-3`,children:E.map((e,t)=>y(`div`,{className:`space-y-2`,children:[b(`div`,{className:`text-xs uppercase tracking-wide text-base-content/60`,children:e}),b(S,{size:e,min:0,max:100,defaultValue:35+t*20})]},e))})]}),code:`<Range size="xs" min={0} max={100} defaultValue={30} />
...
<Range size="large" min={0} max={100} defaultValue={75} />`});m(()=>l(e,s,U))});let W=h(`rue:component:anchor`);e(s,W),t(()=>{let e=p(D,{title:`受控提交`,summary:`拖动中用 onValueChange 更新即时反馈，松手后用 onValueCommit 记录确认值。`,tab:I,preview:()=>b(j,{}),code:`const seats = ref(12)
...
/>`});m(()=>l(e,s,W))});let G=h(`rue:component:anchor`);e(s,G),t(()=>{let e=p(D,{title:`Range with custom color and no fill`,summary:`基础的自定义 CSS 变量方案继续可用；语义 API 不会挡住底层变量覆写。`,tab:L,preview:()=>b(P,{}),code:`<Range
  min={0}
  max={100}
  defaultValue={40}
