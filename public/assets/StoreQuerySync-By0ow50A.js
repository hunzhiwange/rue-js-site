...
`,Demo:()=>{let{route:a,demo:f,historyRecords:te,historyRecorderCleanup:w,appendHistoryRecord:T,currentPath:E,currentPage:D,expectedQuery:O,resetStoreQuery:ie,clearAllUrlParams:ae}=S(`useSetup:0:0`,()=>u(()=>{let e=re(),n=we(),i=S(`ref:1:4`,()=>t([])),a=S(`ref:1:5`,()=>t(null)),o=(e,t)=>{be+=1,i.value=[{id:be,kind:e,href:t},...i.value].slice(0,_e)};v(()=>{a.value=Ce(o)}),l(()=>{a.value?.(),n.clampEffect.dispose(),n.root.dispose()});let s=e.get()?.path||`/examples/store-query-sync`,c=S(`computed:1:6`,()=>r(()=>Math.min(Math.max(1,n.store.page),n.totalPages.get()))),u=S(`computed:1:7`,()=>r(()=>X(n.store))),d=()=>{n.store.search=``,n.store.tab=`all`,n.store.page=1};return{route:e,demo:n,historyRecords:i,historyRecorderCleanup:a,appendHistoryRecord:o,currentPath:s,currentPage:c,expectedQuery:u,resetStoreQuery:d,clearAllUrlParams:()=>{if(d(),typeof window>`u`||!window.history)return;let e=new URL(window.location.href);e.search=``,window.history.replaceState(window.history.state,``,`${e.pathname}${e.search}${e.hash}`)}}}));return p(t=>{let r=h(`div`,t);s(r,`grid gap-6`);let a=h(`div`,r);n(r,a),s(a,`card bg-base-100 shadow`);let l=h(`div`,a);n(a,l),s(l,`card-body gap-4`);let u=h(`div`,l);n(l,u),s(u,`flex flex-wrap items-start justify-between gap-3`);let v=h(`div`,u);n(u,v),s(v,`space-y-2`);let S=h(`h2`,v);n(v,S),s(S,`text-2xl font-semibold`),n(S,y(`Store、Router、Query Sync 串联演示`));let re=h(`p`,v);n(v,re),s(re,`max-w-3xl text-sm leading-6 text-base-content/75`),n(re,y(`搜索框走 500ms debounce，分类和分页走 throttle + history push。页面路径由 Router 控制，查询参数由 Store Query Sync 控制，二者共享同一条可分享链接。`));let w=h(`div`,u);n(u,w),s(w,`flex flex-wrap gap-2 text-xs`);let T=h(`span`,w);n(w,T),s(T,`rounded-full bg-base-200 px-3 py-1 font-medium`),n(T,y(`当前路由：`));let k=c(`rue:slot:anchor`);n(T,k),x(()=>{let t=E;e(()=>g(t,T,k))});let A=h(`span`,w);n(w,A),s(A,`rounded-full bg-base-200 px-3 py-1 font-medium`),n(A,y(`预期 Query：`));let oe=c(`rue:slot:anchor`);n(A,oe),x(()=>{let t=O.get();e(()=>g(t,A,oe))});let j=h(`div`,l);n(l,j),s(j,`flex flex-wrap gap-2`);let M=h(`a`,j);n(j,M),x(()=>{m(M,`href`,String(C.__rueHref(`${E}?q=router&tab=router&page=1`)))}),i(M,`click`,e=>C.__rueOnClick(e,`${E}?q=router&tab=router&page=1`,!1)),s(M,`btn btn-sm btn-outline`),n(M,y(`Router 预设`));let N=h(`a`,j);n(j,N),x(()=>{m(N,`href`,String(C.__rueHref(`${E}?q=sync&tab=store&page=1`)))}),i(N,`click`,e=>C.__rueOnClick(e,`${E}?q=sync&tab=store&page=1`,!1)),s(N,`btn btn-sm btn-outline`),n(N,y(`Store 预设`));let P=h(`a`,j);n(j,P),x(()=>{m(P,`href`,String(C.__rueHref(`${E}?q=render&tab=runtime&page=1`)))}),i(P,`click`,e=>C.__rueOnClick(e,`${E}?q=render&tab=runtime&page=1`,!1)),s(P,`btn btn-sm btn-outline`),n(P,y(`Runtime 预设`));let F=h(`button`,j);n(j,F),s(F,`btn btn-sm btn-ghost`),i(F,`click`,ie),n(F,y(`清空 Query`));let I=h(`button`,j);n(j,I),s(I,`btn btn-sm btn-ghost`),i(I,`click`,ae),n(I,y(`清理所有 URL 参数`));let L=h(`div`,r);n(r,L),s(L,`grid gap-6`);let se=h(`div`,L);n(L,se),s(se,`card bg-base-100 shadow`);let R=h(`div`,se);n(se,R),s(R,`card-body gap-4`);let z=h(`label`,R);n(R,z),s(z,`form-control gap-2`);let ce=h(`span`,z);n(z,ce),s(ce,`label-text font-medium`),n(ce,y(`搜索（debounce 500ms）`));let B=h(`input`,z);n(z,B),s(B,`input input-bordered`),x(()=>{o(B,f.store.search)}),i(B,`input`,e=>{f.store.search=e.target.value,f.store.page=1}),m(B,`placeholder`,`例如：router / sync / render`);let V=h(`div`,R);n(R,V),s(V,`grid gap-2`);let H=h(`span`,V);n(V,H),s(H,`label-text font-medium`),n(H,y(`分类（throttle 180ms + push）`));let U=h(`div`,V);n(V,U),s(U,`flex flex-wrap gap-2`);let le=c(`rue:list:start`),ue=c(`rue:list:end`);n(U,le),n(U,ue);let W=new Map;x(()=>{W=ne({items:ye||[],getKey:(e,t)=>e.id,elements:W,parent:U,before:ue,singleRoot:!0,trackIndex:!1,start:le,renderItem:(t,r,a,o,l)=>{g(p(()=>{let r=b(),a=h(`button`,r);n(r,a),x(()=>{m(a,`key`,String(t.id))}),x(()=>{s(a,`btn btn-sm ${f.store.tab===t.id?`btn-primary`:`btn-outline`}`)}),i(a,`click`,()=>{f.store.tab=t.id,f.store.page=1});let o=c(`rue:slot:anchor`);return n(a,o),x(()=>{let n=t.label;e(()=>g(n,a,o))}),r}),r,a)}})});let de=h(`p`,V);n(V,de),s(de,`text-xs leading-5 text-base-content/60`);let fe=d(de);n(de,fe),x(()=>{ee(fe,ye.find(e=>e.id===f.store.tab)?.hint)});let G=h(`div`,R);n(R,G),s(G,`grid gap-2`);let pe=h(`span`,G);n(G,pe),s(pe,`label-text font-medium`),n(pe,y(`分页（throttle 180ms + push）`));let K=h(`div`,G);n(G,K),s(K,`flex items-center gap-2`);let q=h(`button`,K);n(K,q),s(q,`btn btn-sm btn-outline`),x(()=>{_(q,D.get()<=1)}),i(q,`click`,()=>{f.store.page=Math.max(1,D.get()-1)}),n(q,y(`上一页`));let J=h(`span`,K);n(K,J),s(J,`min-w-24 text-center text-sm font-medium`),n(J,y(`第 `));let me=c(`rue:slot:anchor`);n(J,me),x(()=>{let t=D.get();e(()=>g(t,J,me))}),n(J,y(` / `));let he=c(`rue:slot:anchor`);n(J,he),x(()=>{let t=f.totalPages.get();e(()=>g(t,J,he))}),n(J,y(` 页`));let ge=h(`button`,K);n(K,ge),s(ge,`btn btn-sm btn-outline`),x(()=>{_(ge,D.get()>=f.totalPages.get())}),i(ge,`click`,()=>{f.store.page=Math.min(f.totalPages.get(),D.get()+1)}),n(ge,y(`下一页`));let Y=h(`div`,R);n(R,Y),s(Y,`rounded-2xl border border-base-200 bg-base-200/40 p-4 text-sm leading-6 text-base-content/70`);let _e=h(`p`,Y);n(Y,_e),n(_e,y(`本地状态会立刻刷新卡片列表。`));let ve=h(`p`,Y);n(Y,ve),n(ve,y(`地址栏会按 debounce 或 throttle 的规则延后追上。`));let be=h(`p`,Y);n(Y,be),n(be,y(`上面的预设链接会保持同一路由，只替换 query，用浏览器后退能看到 push 历史回放。`));let xe=h(`p`,Y);n(Y,xe),n(xe,y(`下方记录面板会直接捕获 pushState、replaceState 和 popstate，方便确认“下一页”是否真的入栈。`));let X=h(`div`,R);n(R,X),s(X,`grid gap-2`);let Z=h(`div`,X);n(X,Z),s(Z,`flex flex-wrap items-center justify-between gap-2`);let Ce=h(`span`,Z);n(Z,Ce),s(Ce,`label-text font-medium`),n(Ce,y(`URL 写入记录`));let we=h(`span`,Z);n(Z,we),s(we,`text-xs text-base-content/60`),n(we,y(`分类 / 分页走 pushState，搜索走 replaceState。`));let Te=c(`rue:slot:anchor`);n(X,Te),x(()=>{let t=te.value.length?p(()=>{let t=b(),r=h(`ul`,t);n(t,r),s(r,`rounded-2xl border border-base-200 bg-base-100`);let i=c(`rue:list:start`),a=c(`rue:list:end`);n(r,i),n(r,a);let o=new Map;return x(()=>{o=ne({items:te.value||[],getKey:(e,t)=>e.id,elements:o,parent:r,before:a,singleRoot:!0,trackIndex:!1,start:i,renderItem:(t,r,i,a,o)=>{g(p(()=>{let r=b(),i=h(`li`,r);n(r,i),x(()=>{m(i,`key`,String(t.id))}),s(i,`flex flex-wrap items-center gap-2 border-b border-base-200 px-4 py-3 text-sm last:border-b-0`);let a=h(`span`,i);n(i,a),x(()=>{s(a,`badge badge-sm ${t.kind===`push`?`badge-primary`:t.kind===`replace`?`badge-ghost`:`badge-outline`}`)});let o=c(`rue:slot:anchor`);n(a,o),x(()=>{let n=Se(t.kind);e(()=>g(n,a,o))});let l=h(`span`,i);n(i,l),s(l,`font-mono text-xs text-base-content/70`);let u=c(`rue:slot:anchor`);return n(l,u),x(()=>{let n=t.href;e(()=>g(n,l,u))}),r}),r,i)}})}),t}):p(()=>{let e=b(),t=h(`div`,e);return n(e,t),s(t,`rounded-2xl border border-dashed border-base-300 p-4 text-sm text-base-content/60`),n(t,y(`暂无 URL 写入记录。点击“下一页”或切换分类后，会在这里看到入栈结果。`)),e});e(()=>g(t,X,Te))});let Ee=h(`div`,L);n(L,Ee),s(Ee,`card bg-base-100 shadow`);let De=h(`div`,Ee);n(Ee,De),s(De,`card-body gap-4`);let Oe=h(`div`,De);n(De,Oe),s(Oe,`flex flex-col gap-3`);let ke=h(`div`,Oe);n(Oe,ke);let Ae=h(`h3`,ke);n(ke,Ae),s(Ae,`text-xl font-semibold`),n(Ae,y(`筛选结果`));let Q=h(`p`,ke);n(ke,Q),s(Q,`text-sm text-base-content/70`),n(Q,y(`共 `));let je=c(`rue:slot:anchor`);n(Q,je),x(()=>{let t=f.filteredItems.get().length;e(()=>g(t,Q,je))}),n(Q,y(` 条，当前展示 `));let Me=c(`rue:slot:anchor`);n(Q,Me),x(()=>{let t=f.visibleItems.get().length;e(()=>g(t,Q,Me))});let Ne=d(Q);n(Q,Ne),ee(Ne,` `),n(Q,y(`条。`));let $=h(`div`,Oe);n(Oe,$),s($,`flex flex-wrap gap-2 text-xs`);let Pe=h(`span`,$);n($,Pe),s(Pe,`rounded-full bg-base-200 px-3 py-1`),n(Pe,y(`search=`));let Fe=c(`rue:slot:anchor`);n(Pe,Fe),x(()=>{let t=f.store.search||`(空)`;e(()=>g(t,Pe,Fe))});let Ie=h(`span`,$);n($,Ie),s(Ie,`rounded-full bg-base-200 px-3 py-1`),n(Ie,y(`tab=`));let Le=c(`rue:slot:anchor`);n(Ie,Le),x(()=>{let t=f.store.tab;e(()=>g(t,Ie,Le))});let Re=h(`span`,$);n($,Re),s(Re,`rounded-full bg-base-200 px-3 py-1`),n(Re,y(`page=`));let ze=c(`rue:slot:anchor`);n(Re,ze),x(()=>{let t=D.get();e(()=>g(t,Re,ze))});let Be=c(`rue:slot:anchor`);return n(De,Be),x(()=>{let t=f.visibleItems.get().length?p(()=>{let t=b(),r=h(`div`,t);n(t,r),s(r,`grid gap-3 md:grid-cols-2 xl:grid-cols-3`);let i=c(`rue:list:start`),a=c(`rue:list:end`);n(r,i),n(r,a);let o=new Map;return x(()=>{o=ne({items:f.visibleItems.get()||[],getKey:(e,t)=>e.id,elements:o,parent:r,before:a,singleRoot:!0,trackIndex:!1,start:i,renderItem:(t,r,i,a,o)=>{g(p(()=>{let r=b(),i=h(`article`,r);n(r,i),x(()=>{m(i,`key`,String(t.id))}),s(i,`rounded-2xl border border-base-200 bg-gradient-to-br from-base-100 to-base-200/70 p-4`);let a=h(`div`,i);n(i,a),s(a,`mb-3 flex items-center justify-between gap-3`);let o=h(`span`,a);n(a,o),s(o,`badge badge-outline`);let l=c(`rue:slot:anchor`);n(o,l),x(()=>{let n=t.badge;e(()=>g(n,o,l))});let u=h(`span`,a);n(a,u),s(u,`text-xs text-base-content/50`);let d=c(`rue:slot:anchor`);n(u,d),x(()=>{let n=t.id;e(()=>g(n,u,d))});let f=h(`h4`,i);n(i,f),s(f,`text-base font-semibold leading-6`);let p=c(`rue:slot:anchor`);n(f,p),x(()=>{let n=t.title;e(()=>g(n,f,p))});let _=h(`p`,i);n(i,_),s(_,`mt-2 text-sm leading-6 text-base-content/70`);let v=c(`rue:slot:anchor`);return n(_,v),x(()=>{let n=t.teaser;e(()=>g(n,_,v))}),r}),r,i)}})}),t}):p(()=>{let e=b(),t=h(`div`,e);return n(e,t),s(t,`rounded-2xl border border-dashed border-base-300 p-8 text-center text-sm text-base-content/60`),n(t,y(`当前筛选没有命中结果。尝试清空搜索词，或者切回“全部”分类。`)),e});e(()=>g(t,De,Be))}),r})},codeCardClassName:`h-[520px] md:h-[1220px]`});export{Te as default};