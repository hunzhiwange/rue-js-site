...
hide()`,ge=()=>{let{hookExtraSeed:e,hookApi:i,hookHolder:a}=y(`useSetup:0:0`,()=>d(()=>{let e=y(`ref:1:0`,()=>r(0)),[t,n]=$.useMessage({getContainer:!1,placement:`top-end`,top:12,maxCount:3,gap:12,className:`absolute`,duration:3,closable:!0});return{hookExtraSeed:e,hookApi:t,hookHolder:n}}));return s(r=>{let o=l(),s=g(`rue:component:anchor`);return t(o,s),n(()=>{let t=m(ce,{minHeight:`20rem`,content:S(c,{children:[x(`div`,{className:`absolute inset-0 bg-[radial-gradient(circle_at_top_right,_color-mix(in_srgb,var(--color-info)_16%,transparent),_transparent_38%),radial-gradient(circle_at_bottom_left,_color-mix(in_srgb,var(--color-success)_18%,transparent),_transparent_36%)]`}),a,S(`div`,{className:`absolute inset-x-4 top-4 flex flex-wrap gap-2`,children:[x(`button`,{type:`button`,className:`btn btn-sm rounded-full`,onClick:()=>{i.loading({key:`sync-flow`,content:`正在同步 design tokens...`,duration:0})},children:`open loading`}),x(`button`,{type:`button`,className:`btn btn-sm rounded-full`,onClick:()=>{i.success({key:`sync-flow`,content:`同步完成，缓存已刷新。`,duration:2})},children:`update by key`}),x(`button`,{type:`button`,className:`btn btn-sm btn-ghost rounded-full`,onClick:()=>{e.value+=1,i.warning({key:`review-${e.value}`,content:`审阅提醒 #${e.value}`,duration:0})},children:`add another`}),x(`button`,{type:`button`,className:`btn btn-sm btn-ghost rounded-full`,onClick:()=>{i.destroy(`sync-flow`)},children:`destroy key`})]}),x(`div`,{className:`absolute inset-x-4 bottom-4 rounded-[1rem] border border-base-300 bg-base-100/85 px-4 py-3 text-sm text-base-content/70 backdrop-blur`,children:`点击上方按钮后，消息会留在当前示例盒子内，并支持同 key 更新与局部销毁。`})]})});h(()=>u(t,o,s))}),o})},_e=()=>{let{promiseBatch:e,activePromiseHandle:i,promiseApi:a,promiseHolder:o}=y(`useSetup:0:0:dup1`,()=>d(()=>{let e=y(`ref:1:1`,()=>r(0)),t=y(`ref:1:2`,()=>r(void 0)),[n,i]=$.useMessage({getContainer:!1,placement:`bottom-start`,inset:{x:12,y:80},maxCount:2,gap:12,className:`absolute`,closable:!0});return{promiseBatch:e,activePromiseHandle:t,promiseApi:n,promiseHolder:i}}));return s(r=>{let s=l(),d=g(`rue:component:anchor`);return t(s,d),n(()=>{let t=m(ce,{minHeight:`18rem`,content:S(c,{children:[S(`div`,{className:`absolute inset-x-4 top-4 flex flex-wrap gap-2`,children:[x(`button`,{type:`button`,className:`btn btn-sm rounded-full`,onClick:()=>{i.value?.(),e.value+=1;let t=e.value,n=a.loading({key:`promise-flow`,content:S(`span`,{className:`inline-flex items-center gap-2`,children:[S(`span`,{className:`font-medium`,children:[`正在发布批次 #`,t]}),x(`span`,{className:`badge badge-ghost badge-sm`,children:`await close`})]}),duration:0});i.value=n,n.then(()=>{a.success({key:`promise-flow-done-${t}`,content:`批次 #${t} 已完成，promise 已 resolve。`,duration:2})})},children:`开始批次`}),x(`button`,{type:`button`,className:`btn btn-sm btn-outline rounded-full`,onClick:()=>{i.value?.(),i.value=void 0},children:`完成批次`})]}),o,x(`div`,{className:`absolute inset-x-4 bottom-4 rounded-[1rem] border border-base-300 bg-base-100/85 px-4 py-3 text-sm text-base-content/70 backdrop-blur`,children:`点击“开始批次”创建 loading handle，再点击“完成批次”观察 then resolve。`})]})});h(()=>u(t,s,d))}),s})},ve=`import { Message } from '@rue-js/design'
