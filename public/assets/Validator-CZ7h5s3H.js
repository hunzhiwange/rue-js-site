...
</form>`});h(()=>u(e,c,V))});let H=g(`rue:component:anchor`);e(c,H),t(()=>{let e=m(R,{title:`Different validator hosts`,summary:`展示基础 select 和 textarea 场景，并补上 checkbox / toggle 这种同样依赖 validator 的宿主。`,tab:i,preview:()=>S(`div`,{className:`grid w-full max-w-4xl gap-4 md:grid-cols-2`,children:[S(`div`,{className:`grid gap-2`,children:[S(L,{as:`select`,appearance:`select`,required:!0,children:[x(`option`,{value:``,children:`Choose:`}),x(`option`,{value:`tabs`,children:`Tabs`}),x(`option`,{value:`spaces`,children:`Spaces`})]}),x(L.Hint,{children:`Required`})]}),S(`div`,{className:`grid gap-2`,children:[x(L,{as:`textarea`,appearance:`textarea`,rows:4,placeholder:`Project notes`,required:!0}),x(L.Hint,{children:`Required`})]}),S(`label`,{className:`flex items-center gap-3 rounded-box border border-base-300 bg-base-100 px-4 py-3`,children:[x(L,{appearance:`checkbox`,type:`checkbox`,required:!0}),x(`span`,{className:`text-sm text-base-content`,children:`Accept release checklist`})]}),S(`label`,{className:`flex items-center justify-between gap-3 rounded-box border border-base-300 bg-base-100 px-4 py-3`,children:[x(`span`,{className:`text-sm text-base-content`,children:`Enable deploy gate`}),x(L,{appearance:`toggle`,type:`checkbox`,required:!0})]})]}),code:`<Validator as="select" appearance="select" required={true}>
...
<Validator appearance="toggle" type="checkbox" required={true} />`});h(()=>u(e,c,H))});let W=g(`rue:component:anchor`);e(c,W),t(()=>{let e=m(R,{title:`Hidden hint and rule list`,summary:`提交、失焦或输入时都会同步原生 invalid 状态，隐藏 hint 和多行规则列表现在会在首次校验失败后正确出现。`,tab:a,preview:()=>S(`div`,{className:`grid w-full max-w-3xl gap-4 md:grid-cols-2`,children:[x(U,{}),x(ee,{})]}),code:`const syncNativeValidity = (event: Event) => {
...
</form>`});h(()=>u(e,c,W))});let G=g(`rue:component:anchor`);e(c,G),t(()=>{let e=m(R,{title:`Manual status and size`,summary:`status 适合展示服务端或异步校验反馈；size 则让同一套 Validator 能直接落到不同密度的表单里。`,tab:o,preview:()=>S(`div`,{className:`grid w-full max-w-4xl gap-4 md:grid-cols-3`,children:[x(L.Field,{id:`validator-status-error`,label:`Workspace slug`,hint:`Already taken on the edge cluster`,appearance:`input`,size:`sm`,status:`error`,defaultValue:`rue-design`}),x(L.Field,{id:`validator-status-warning`,label:`Notification email`,hint:`MX record is still being verified`,appearance:`input`,status:`warning`,defaultValue:`team@rue.dev`}),S(L.Field,{id:`validator-status-success`,label:`Release channel`,hint:`Synced with the latest deployment policy`,appearance:`select`,as:`select`,size:`lg`,status:`success`,children:[x(`option`,{children:`Stable`}),x(`option`,{children:`Canary`})]})]}),code:`<Validator.Field
...
/>
`});h(()=>u(e,c,K))});let q=g(`rue:component:anchor`);e(c,q),t(()=>{let e=m(R,{title:`Form recipe`,summary:`把基础散装示例 融合成一个更接近真实业务的登录表单，同时保持 Validator 只负责视觉反馈。`,tab:v,preview:()=>S(`form`,{autocomplete:`off`,className:`grid w-full max-w-sm gap-4 rounded-box border border-base-300 bg-base-200 p-5`,onSubmit:e=>e.preventDefault(),children:[x(L.Field,{id:`validator-login-email`,label:`Email`,hint:`Required`,appearance:`input`,type:`email`,required:!0,placeholder:`mail@site.com`,hideHintWhenValid:!0}),x(L.Field,{id:`validator-login-password`,label:`Password`,hint:[`At least 8 characters`,`Contains uppercase, lowercase and a number`],appearance:`input`,type:`password`,required:!0,minLength:8,pattern:`(?=.*\\\\d)(?=.*[a-z])(?=.*[A-Z]).{8,}`,placeholder:`Password`,hideHintWhenValid:!0}),S(`div`,{className:`grid gap-2 sm:grid-cols-2`,children:[x(w,{color:`primary`,htmlType:`submit`,block:!0,children:`Login`}),x(w,{type:`text`,htmlType:`reset`,block:!0,children:`Reset`})]})]}),code:`<form className="grid w-full max-w-sm gap-4 rounded-box border border-base-300 bg-base-200 p-5">
  <Validator.Field
