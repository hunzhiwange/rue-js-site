...
)`,J=p(`ref:1:0`,()=>t(`preview`)),Y=p(`ref:1:1`,()=>t(`preview`)),X=p(`ref:1:2`,()=>t(`preview`)),Z=p(`ref:1:3`,()=>t(`preview`)),Q=p(`ref:1:4`,()=>t(`preview`)),$=p(`ref:1:5`,()=>t(`preview`)),ee=[{prop:`align`,description:`内容对齐方式，支持居中展示或左对齐的流程详情布局`,type:`'center' | 'start'`,defaultValue:`center`},{prop:`bodyClassName / bodyStyle`,description:`children 内容面板的类名与样式`,type:`string / any`,defaultValue:`-`},{prop:`bordered`,description:`是否显示边框；outline 变体会强制保留边框视觉`,type:`boolean`,defaultValue:`true`},{prop:`children`,description:`结果页下方的扩展说明区域，可放详情、表单或下一步信息`,type:`any`,defaultValue:`-`},{prop:`contentClassName / contentStyle`,description:`内部纵向布局容器的类名与样式`,type:`string / any`,defaultValue:`-`},{prop:`extra`,description:`操作区，可传单个节点或按钮数组`,type:`any`,defaultValue:`-`},{prop:`icon`,description:`自定义图标或插画；传 null / false 可隐藏图标区`,type:`any`,defaultValue:`按 status 自动生成`},{prop:`showIcon`,description:`显式控制默认图标区显隐`,type:`boolean`,defaultValue:`true`},{prop:`size`,description:`控制标题、图标和异常插画尺寸`,type:`'sm' | 'md' | 'lg'`,defaultValue:`md`},{prop:`status`,description:`语义状态，兼容操作结果与 403/404/500 异常页`,type:`'success' | 'info' | 'warning' | 'error' | 403 | 404 | 500 | '403' | '404' | '500'`,defaultValue:`info`},{prop:`subTitle`,description:`辅助说明文案；403/404/500 未传时会自动补默认副标题`,type:`any`,defaultValue:`-`},{prop:`title`,description:`主标题；403/404/500 未传时会自动补默认标题`,type:`any`,defaultValue:`-`},{prop:`variant`,description:`Rue 风格外观，适配仪表盘、表单完成态与空页面`,type:`'surface' | 'soft' | 'outline'`,defaultValue:`surface`}],te=()=>c(t=>{let d=n(),p=o(`rue:component:anchor`);return i(d,p),u(f(b,{children:c(()=>{let t=n(),c=e(`div`,t);i(t,c),s(c,`max-w-none prose prose-sm md:prose-base`);let d=e(`h1`,c);i(c,d),i(d,l(`Result 结果页`));let p=e(`p`,c);i(c,p),s(p,`text-sm mt-3 mb-3`),i(p,l(`Result 用于承载关键操作后的反馈、异常页与阶段完成态。它比 Alert 更完整，也比空状态更适合承接下一步动作。`));let m=e(`p`,c);i(c,m),s(m,`text-sm opacity-75`),i(m,l(`这次补齐参考了成熟结果页组件的能力面，但视觉保持 Rue 自己的圆角面板、柔和光晕和较轻的插画结构；除了 success、info、warning、error 之外，也内置了 403、404、500 三种异常状态。`));let v=o(`rue:component:anchor`);i(c,v),a(()=>{let e=f(B,{title:`基础状态`,summary:`覆盖最常见的操作结果场景。四种语义状态共享一套 Result 布局，但通过图标与色彩建立清晰区分。`,tab:J,preview:()=>h(`div`,{className:`grid gap-4 xl:grid-cols-2`,children:[g(z,{size:`sm`,status:`success`,title:`发布成功`,subTitle:`新版本已推送到 12 个边缘节点，预计 40 秒内完成全量刷新。`}),g(z,{size:`sm`,status:`info`,title:`等待同步`,subTitle:`任务已经入队，系统会在资源空闲时继续处理。`}),g(z,{size:`sm`,status:`warning`,title:`仍需人工确认`,subTitle:`检测到配置变更涉及生产环境，请由值班同学完成最终审核。`}),g(z,{size:`sm`,status:`error`,title:`校验失败`,subTitle:`依赖清单中存在 2 个未解决的版本冲突，请修正后重新提交。`})]}),code:`import { Result } from '@rue-js/design'
...
`});r(()=>u(e,c,v))});let b=o(`rue:component:anchor`);i(c,b),a(()=>{let e=f(B,{title:`异常状态页`,summary:`403、404、500 内置默认标题、副标题和插画，适合直接作为独立页面或路由兜底页使用。`,tab:Y,preview:()=>h(`div`,{className:`grid gap-5`,children:[g(z,{status:403,extra:g(_,{color:`primary`,children:`申请访问`}),className:`min-h-[26rem]`}),h(`div`,{className:`grid gap-5 xl:grid-cols-2`,children:[g(z,{status:404,size:`sm`,extra:[g(_,{color:`primary`,children:`返回首页`},`home`),g(_,{type:`outlined`,children:`搜索文档`},`search`)]}),g(z,{status:500,size:`sm`,extra:[g(_,{color:`primary`,children:`重试请求`},`retry`),g(_,{type:`text`,children:`查看日志`},`log`)]})]})]}),code:`import { Button, Result } from '@rue-js/design'
...
`});r(()=>u(e,c,b))});let x=o(`rue:component:anchor`);i(c,x),a(()=>{let e=f(B,{title:`操作区与扩展内容`,summary:`extra 负责下一步动作，children 负责补充上下文或明细面板，适合复杂业务流收尾页。`,tab:X,preview:()=>g(z,{status:`success`,title:`云主机已开通`,subTitle:`订单号：RU20260510-0032。资源已完成基础检测，建议立即配置告警与自动快照策略。`,extra:[g(_,{color:`primary`,children:`前往控制台`},`console`),g(_,{type:`outlined`,children:`配置快照`},`snapshot`)],children:h(`div`,{className:`grid gap-3 md:grid-cols-3`,children:[h(`div`,{className:`rounded-2xl border border-base-300 bg-base-100 p-4`,children:[g(`div`,{className:`text-xs uppercase tracking-[0.22em] opacity-55`,children:`实例规格`}),g(`div`,{className:`mt-2 text-lg font-semibold`,children:`4C8G / 华东 2`}),g(`div`,{className:`mt-1 text-sm opacity-65`,children:`自动扩容策略已启用`})]}),h(`div`,{className:`rounded-2xl border border-base-300 bg-base-100 p-4`,children:[g(`div`,{className:`text-xs uppercase tracking-[0.22em] opacity-55`,children:`公网访问`}),h(`div`,{className:`mt-2 flex items-center gap-2 text-lg font-semibold`,children:[`已开启`,g(y,{color:`success`,children:`HTTPS`})]}),g(`div`,{className:`mt-1 text-sm opacity-65`,children:`默认指向 preview.rue.dev`})]}),h(`div`,{className:`rounded-2xl border border-base-300 bg-base-100 p-4`,children:[g(`div`,{className:`text-xs uppercase tracking-[0.22em] opacity-55`,children:`提醒`}),h(`ul`,{className:`mt-2 m-0 pl-5 text-sm leading-7 opacity-80`,children:[g(`li`,{children:`建议补充管理员手机号`}),g(`li`,{children:`七天后会触发成本周报`})]})]})]})}),code:`import { Badge, Button, Result } from '@rue-js/design'
...
`});r(()=>u(e,c,x))});let S=o(`rue:component:anchor`);i(c,S),a(()=>{let e=f(B,{title:`自定义图标与无图标模式`,summary:`你可以把 Result 用成更品牌化的完成态，也可以完全拿掉 icon，仅保留标题、动作和补充文案。`,tab:Z,preview:()=>h(`div`,{className:`grid gap-5 xl:grid-cols-2`,children:[g(z,{status:`info`,icon:g(H,{}),title:`迁移任务已排定`,subTitle:`冷数据会在凌晨窗口期分批迁移到对象存储，预计耗时 18 分钟。`,extra:g(_,{color:`primary`,children:`查看任务看板`}),variant:`soft`}),g(z,{status:`warning`,showIcon:!1,title:`你正在使用只读副本`,subTitle:`当前环境不允许直接写入。若需要继续调试，请先切回可写分支或申请沙箱环境。`,extra:[g(_,{type:`outlined`,children:`切换分支`},`branch`),g(_,{type:`text`,children:`申请沙箱`},`sandbox`)],variant:`outline`})]}),code:`import { Button, Result } from '@rue-js/design'
...
`});r(()=>u(e,c,S))});let C=o(`rue:component:anchor`);i(c,C),a(()=>{let e=f(B,{title:`视觉变体与流程排布`,summary:`surface、soft、outline 适合不同密度的页面；align=start 更适合承载表单结果、审核流和运维说明。`,tab:Q,preview:()=>h(`div`,{className:`grid gap-5`,children:[g(z,{status:`success`,icon:g(W,{}),title:`安全基线已更新`,subTitle:`所有高危项已完成修复，系统会在下一轮巡检中重新生成合规快照。`,variant:`surface`}),h(`div`,{className:`grid gap-5 xl:grid-cols-2`,children:[g(z,{status:`info`,size:`sm`,variant:`soft`,icon:g(U,{}),title:`推荐下一步：整理路由入口`,subTitle:`新组件已经落地，建议同步补一条导航入口和使用示例，降低首次发现成本。`,align:`start`,extra:g(_,{color:`primary`,children:`查看接入清单`})}),g(z,{status:`error`,size:`sm`,variant:`outline`,title:`回滚未完成`,subTitle:`2 个节点仍在回滚旧版本，建议先暂停流量切换，待日志完全一致后再继续。`,align:`start`,extra:g(_,{type:`outlined`,children:`打开故障时间线`}),children:h(`div`,{className:`grid gap-2 text-sm leading-7 opacity-80`,children:[g(`div`,{children:`node-sh-02: checksum mismatch`}),g(`div`,{children:`node-bj-05: waiting runtime restart`})]})})]})]}),code:`import { Button, Result } from '@rue-js/design'
...
`});r(()=>u(e,c,w))});let T=e(`h2`,c);i(c,T),s(T,`mt-10`),i(T,l(`API`));let E=o(`rue:component:anchor`);i(c,E),a(()=>{let e=f(V,{rows:ee});r(()=>u(e,c,E))});let D=e(`p`,c);return i(c,D),s(D,`text-sm opacity-70 mt-4`),i(D,l(`除了上表 props，组件还暴露了 Result.PRESENTED_IMAGE_403、Result.PRESENTED_IMAGE_404 和 Result.PRESENTED_IMAGE_500 三个静态插画组件，便于你在更复杂的空壳页面里单独复用。`)),t})}),d,p),d});export{te as default};